...
public class RedisCallBackStorage(IConnectionMultiplexer redis)
{
    ...
    public async Task<string> StoreCallBackAsync(NoteCallBackData callBackData)
    {
    ...
    public async Task<NoteCallBackData?> GetCallBackAsync(string key)
    {
        var json = await _db.StringGetAsync(key);
        if (json.IsNullOrEmpty) return null;
        return JsonSerializer.Deserialize<NoteCallBackData>(json!);
    }
}